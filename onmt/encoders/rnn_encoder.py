...
class RNNEncoderV2(EncoderBase):
    """ A generic recurrent neural network encoder.
    ...
    def forward(self, src, context, lengths=None):
        """See :func:`EncoderBase.forward()`"""
        self._check_args(src, lengths)
        ...
        if self.use_bridge:
            encoder_final = self._bridge(encoder_final, context)
        return encoder_final, memory_bank, lengths

    def _initialize_bridge(self, rnn_type,
                           enc_hidden_size, context_size,
                           dec_hidden_size,
                           num_layers):
    ...
    def _bridge(self, hidden, context):
        """Forward hidden state through bridge."""
        def bottle_hidden(linear, states, context):
            """
            Transform from 3D to 2D, apply linear and return initial size
            """
        ...
        if isinstance(hidden, tuple):  # LSTM
            ...
            outs = bottle_hidden(self.bridge[0], hidden, context)
        return outs
