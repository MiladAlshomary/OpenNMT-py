...
class ContextTranslatorViaAttn(ContextTranslator):
    ...
    def __init__(
        ...
        super(ContextTranslatorViaAttn, self).__init__(model, fields, src_reader, tgt_reader, gpu, n_best, min_length, max_length,
            ratio, beam_size, random_sampling_topk, random_sampling_temp, stepwise_penalty, dump_beam, block_ngram_repeat, ignore_when_blocking,
            replace_unk, constraint_file, phrase_table, data_type, verbose, report_bleu, report_rouge, report_time, copy_attn,
    ...
    def _translate_batch(
        ...
        feats_proj = tile(feats_proj.transpose(0,1), beam_size)
    ...
    def _decode_and_generate(
        ...
        # in case of inference tgt_len = 1, batch = beam times batch_size
        # in case of Gold Scoring tgt_len = actual length, batch = 1 batch
        dec_out, context_outs, dec_attn = self.model.decoder(
            decoder_in, memory_bank, context_proj, memory_lengths=memory_lengths, step=step
        )
